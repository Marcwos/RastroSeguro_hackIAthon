...
export function RecurringEntitiesList({ entities, limit = 5 }: { entities: RecurringEntity[]; limit?: number }) {
  ...
  if (!top.length) return <p className="text-sm text-muted-foreground">No hay entidades recurrentes para este caso.</p>

  return (
    <div className="space-y-2">
      {top.map((item) => (
        <div key={item.key} className="border border-border bg-[var(--surface-low)] p-3">
          <p className="label-mono text-muted-foreground">{item.type.toUpperCase()}</p>
