  ...
  if (!selectedClaim) {
    return (
      <div className="flex h-full flex-col items-center justify-center gap-4 text-muted-foreground">
        ...
        <button onClick={() => setCurrentStep(1)} className="bg-primary px-4 py-2 text-sm text-white">Volver al Paso 1</button>
      </div>
    )
  }
    ...
    <section className="px-4 py-8 lg:px-8">
      <div className="mx-auto max-w-7xl space-y-5">
        <header className="flex flex-col justify-between gap-4 md:flex-row md:items-end">
          <div>
            <span className="label-mono uppercase tracking-widest text-muted-foreground">Intelligence Report</span>
            <h1 className="display-heading text-3xl lg:text-4xl">Paso 3: Evaluación de Riesgo</h1>
            <p className="mt-1 text-muted-foreground">Resumen ejecutivo: riesgo, razones principales y acción.</p>
          </div>
        ...
        <div className="grid grid-cols-12 gap-4">
            ...
            <div className="grid gap-6 md:grid-cols-[220px_1fr] md:items-center">
              <div className="flex justify-center">
                <div className="relative shrink-0">
                  <svg className="h-48 w-48 -rotate-90">
                    <circle cx="96" cy="96" r="88" fill="transparent" stroke="#e0e3e5" strokeWidth="12" />
              ...
              <div className="space-y-4">
                <div className="flex flex-wrap items-center gap-2">
                  <span className="bg-destructive px-4 py-1 label-mono-md font-bold uppercase text-white">{getRiskLabel(nivelRiesgo)}</span>
                  <span className="border border-border bg-[var(--surface-low)] px-3 py-1 label-mono text-muted-foreground">{selectedClaim.id_siniestro}</span>
                </div>
                <p className="text-base leading-relaxed">{explicacion}</p>
                <div className="border-l-4 border-destructive bg-[var(--error-container)] p-4 text-[var(--on-error-container)]">
          ...
          <section className="institutional-card col-span-12 p-6">
            <div className="mb-4 flex flex-col justify-between gap-2 md:flex-row md:items-end">
              ...
              <button onClick={() => setCurrentStep(4)} className="self-start border border-border px-4 py-2 label-mono-md text-muted-foreground hover:text-primary md:self-auto">
          ...
          <section className="institutional-card col-span-12 p-6 lg:col-span-8">
            <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
              ...
              <button onClick={() => setCurrentStep(4)} className="flex shrink-0 items-center justify-center gap-2 bg-primary px-5 py-3 label-mono-md text-white">
        ...
        <footer className="flex items-center justify-between border-t border-border pt-6">
          <button onClick={() => setCurrentStep(2)} className="flex items-center gap-2 px-4 py-2 label-mono-md text-muted-foreground hover:text-primary"><ArrowLeft className="h-4 w-4" />Anterior</button>
          <button onClick={() => setShowChat(true)} className="flex items-center gap-2 bg-primary px-6 py-2 label-mono-md text-white"><Bot className="h-4 w-4" />Consultar IA<CheckCircle2 className="h-4 w-4" /></button>
        </footer>
      </div>
    </section>
  )
